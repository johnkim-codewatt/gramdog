...
def insert_grammar_rules():
    # 1. DB 초기화 (테이블 생성)
    from database import init_db
    init_db()
    
    # 2. JSON 파일 읽기
    json_path = os.path.join(os.path.dirname(__file__), "elements_of_style.json")
    ...
    for rule in rules:
        try:
            # ON CONFLICT 문을 사용하여 tag_id가 이미 존재하면 업데이트
            cur.execute("""
...
if __name__ == "__main__":
    print("Starting to insert Elements of Style rules into DB...")
